...
void HalfAckTimeout (std::string context, const WifiMacHeader & hdr)
{
  ...
  uint8_t add [6];
  src.CopyTo (add);
  d->nodeLogList[add[5]-1]->ackTimeout++; //the number of nodes should not be larger than 256
}
...
// defaults:
  ...
  d->returnPacket = true;
  d->secondaryPacket = false;
  d->busytone = false;
  bool verbose = false;
  d->phyMode = "OfdmRate6Mbps";

//  d->duplexMode = false;
//  d->captureEffect = false;
...
//  d->phyMode = "OfdmRate12Mbps";
  ...
  std::stringstream ss;
  ss<<"runs/pos_" << (d->fullDuplex ? "duplex": "mimo") << "_nodes_"<<(int)d->numNodesPerAp<<"_aps_"<<(int)d->numAps <<"streams" << d->streamsPerNode <<"downRatio" <<downRatio<<"_run_" << nRun;
  d->positionFileName = ss.str ();
  std::stringstream ss_log;
  ss_log<<"runs/log_"<<(d->fullDuplex ? "duplex": "mimo") <<"_nodes_"<<(int)d->numNodesPerAp<<"_aps_"<<(int)d->numAps <<"streams" << d->streamsPerNode <<"downRatio" <<downRatio<<"_run_" << nRun;
  d->logFileName = ss_log.str ();
  std::stringstream ss_flow;
  ss_flow<<"runs/flow_"<<(d->fullDuplex ? "duplex": "mimo") <<"_nodes_"<< (int)d->numNodesPerAp<<"_aps_"<<(int)d->numAps <<"streams" << d->streamsPerNode<<"downRatio" <<downRatio<<"_run_" << nRun;
  d->flowFileName = ss_flow.str ();

  // MOBILITY
  UniformRandomVariable rand_i;
