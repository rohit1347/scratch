 ...
 *
 * To compile:
 * ./waf --run duplex_multirate
 *
 * To compile with command line(useful for varying parameters):
 * ./waf --run "duplex_multirate --totalTime=0.3s --rateManager=ns3::MinstrelWifiManager"
 *
 * To turn on NS_LOG:
 * export NS_LOG=duplex_multirate=level_all
...
NS_LOG_COMPONENT_DEFINE ("duplex_multirate");
                 ...
                 const FullNqosWifiMacHelper &wifiMac, const FullYansWifiChannelHelper &wifiChannel, const MobilityHelper &mobility)
{
  ...
  FullYansWifiPhyHelper phy = wifiPhy;
  phy.SetChannel (wifiChannel.Create ());

  FullNqosWifiMacHelper mac = wifiMac;
...
//  LogComponentEnable("FullInterferenceHelper", LogLevel(LOG_FUNCTION | LOG_ALL| LOG_PREFIX_TIME | LOG_PREFIX_NODE ) );
  LogComponentEnable("duplex_multirate", LogLevel(LOG_FUNCTION | LOG_ALL | LOG_PREFIX_TIME | LOG_PREFIX_NODE) );

  Experiment experiment;
  experiment = Experiment ("duplex_multirate");
  ...
  Config::SetDefault ("ns3::FullWifiRemoteStationManager::RtsCtsThreshold", StringValue (experiment.GetRtsThreshold ()));
  ...
  FullYansWifiChannelHelper wifiChannel = FullYansWifiChannelHelper::Default ();
  FullSsid ssid = FullSsid ("Testbed");

  wifiMac.SetType ("ns3::FullAdhocWifiMac",
                   "Ssid", FullSsidValue (ssid));
  wifi.SetStandard (FULL_WIFI_PHY_STANDARD_holland);
